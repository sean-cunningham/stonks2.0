...
from app.services.paper.strategy_two_dashboard_service import build_strategy_two_dashboard
from app.services.paper.strategy_two_execute_once import (
    run_emergency_close_open_paper_trade,
    run_strategy_two_paper_execute_once,
)
...
router = APIRouter(prefix="/paper/strategy/spy/strategy-2", tags=["paper"])
...
def _require_paper_app_mode(settings: Settings) -> None:
    if settings.APP_MODE != "paper":
        raise HTTPException(
            ...
            detail="strategy_2_paper_automation_requires_app_mode_paper",
        )


@router.post("/execute-once", response_model=StrategyTwoExecuteOnceResponse)
def execute_strategy_two_paper_once(
    ...
    return run_strategy_two_paper_execute_once(db, context=context, market=market, settings=settings)


@router.get("/dashboard", response_model=StrategyDashboardResponse)
def get_strategy_two_dashboard(
    db: Session = Depends(get_db),
    context: ContextService = Depends(get_context_service),
    market: MarketStoreService = Depends(get_market_service),
...
def get_strategy_two_runtime_status(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().get_status(db, settings=settings)


@router.post("/runtime/pause", response_model=StrategyTwoRuntimeStatusResponse)
def pause_strategy_two_runtime(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().set_paused(db, settings=settings, paused=True)


@router.post("/runtime/resume", response_model=StrategyTwoRuntimeStatusResponse)
def resume_strategy_two_runtime(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().set_paused(db, settings=settings, paused=False)


@router.post("/runtime/entry-enable", response_model=StrategyTwoRuntimeStatusResponse)
def enable_strategy_two_runtime_entry(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().set_runtime_flags(db, settings=settings, entry_enabled=True)


@router.post("/runtime/entry-disable", response_model=StrategyTwoRuntimeStatusResponse)
def disable_strategy_two_runtime_entry(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().set_runtime_flags(db, settings=settings, entry_enabled=False)


@router.post("/runtime/exit-enable", response_model=StrategyTwoRuntimeStatusResponse)
def enable_strategy_two_runtime_exit(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().set_runtime_flags(db, settings=settings, exit_enabled=True)


@router.post("/runtime/exit-disable", response_model=StrategyTwoRuntimeStatusResponse)
def disable_strategy_two_runtime_exit(db: Session = Depends(get_db)) -> StrategyTwoRuntimeStatusResponse:
    settings = get_settings()
    _require_paper_app_mode(settings)
    return get_strategy_two_runtime_coordinator().set_runtime_flags(db, settings=settings, exit_enabled=False)
...
def list_open_paper_positions(db: Session = Depends(get_db)) -> list[PaperTradeResponse]:
    repo = PaperTradeRepository(db)
    rows = repo.list_open(strategy_id=StrategyTwoPaperTradeService.STRATEGY_ID)
...
def list_closed_paper_positions(db: Session = Depends(get_db), limit: int = 100) -> list[PaperTradeResponse]:
    repo = PaperTradeRepository(db)
    rows = repo.list_closed(strategy_id=StrategyTwoPaperTradeService.STRATEGY_ID, limit=min(max(limit, 1), 500))
    return [PaperTradeResponse.model_validate(r) for r in rows]
...
def list_paper_trade_journal(db: Session = Depends(get_db), limit: int = 200) -> list[PaperTradeEventResponse]:
    repo = PaperTradeRepository(db)
    events = repo.list_journal(strategy_id=StrategyTwoPaperTradeService.STRATEGY_ID, limit=min(max(limit, 1), 500))
