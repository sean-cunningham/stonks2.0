"""Strategy 2 paper runtime coordinator with independent entry/exit ticks."""
...
from app.services.paper.strategy_two_execute_once import (
    run_strategy_two_paper_entry_once,
    run_strategy_two_paper_exit_once,
...
class StrategyTwoRuntimeCoordinator:
    def __init__(self) -> None:
        ...
        self._strategy_id = StrategyTwoPaperTradeService.STRATEGY_ID
    ...
    def _build_status(
        ...
        *,
    ...
    ) -> StrategyTwoRuntimeStatusResponse:
        ...
        return StrategyTwoRuntimeStatusResponse(
            strategy_id=self._strategy_id,
            mode=settings.APP_MODE,
            scheduler_enabled=settings.STRATEGY2_PAPER_RUNTIME_ENABLED,
            paused=state.paused,
    ...
    def get_status(self, db: Session, *, settings: Settings) -> StrategyTwoRuntimeStatusResponse:
    ...
    def set_paused(self, db: Session, *, settings: Settings, paused: bool) -> StrategyTwoRuntimeStatusResponse:
        repo = StrategyRuntimeRepository(db)
        state = repo.get_or_create_state(strategy_id=self._strategy_id)
        state.paused = paused
    ...
    def run_tick(
        ...
        *,
        ...
        # manual execute-once endpoint compatibility
        repo = StrategyRuntimeRepository(db)
        state = repo.get_or_create_state(strategy_id=self._strategy_id)
        now = datetime.now(timezone.utc)
        if state.paused or not is_within_spy_rth_et(clock_utc=now):
            return self._build_status(state, settings=settings, clock_utc=now)
        out = run_strategy_two_paper_execute_once(
            db,
            context=context,
            market=market,
            settings=settings,
...
_runtime_coordinator = StrategyTwoRuntimeCoordinator()


def get_strategy_two_runtime_coordinator() -> StrategyTwoRuntimeCoordinator:
