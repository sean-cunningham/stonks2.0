...
class MainLifespanRuntimeSchedulerTests(unittest.TestCase):
    def test_lifespan_starts_and_stops_scheduler_when_enabled(self) -> None:
        async def _run() -> None:
            scheduler_inst = MagicMock()
            streamer = MagicMock()
            settings = Settings(APP_MODE="paper", STRATEGY1_PAPER_RUNTIME_ENABLED=True)
            with (
                ...
                patch("app.main.StrategyOneRuntimeScheduler", return_value=scheduler_inst),
            ):
                async with lifespan(FastAPI()):
                    scheduler_inst.start.assert_called_once()
