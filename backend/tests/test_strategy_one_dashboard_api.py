...
class StrategyOneDashboardApiTests(unittest.TestCase):
    def test_dashboard_route_returns_common_shape(self) -> None:
        ...
        payload = StrategyDashboardResponse(
            ...
            strategy_details={"open_monitor_state_counts": {}},
        )

        with (
            patch("app.api.paper_strategy_one.get_settings", return_value=Settings(APP_MODE="paper")),
            patch("app.api.paper_strategy_one.build_strategy_one_dashboard", return_value=payload),
            patch("app.api.paper_strategy_one.get_context_service", return_value=MagicMock()),
            patch("app.api.paper_strategy_one.get_market_service", return_value=MagicMock()),
            TestClient(app) as client,
        ):
            resp = client.get("/paper/strategy/spy/strategy-1/dashboard")
